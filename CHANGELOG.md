...
Requires the Cinatra WordPress plugin's content abilities (cinatra-ai/wordpress-plugin#81 and cinatra-ai/wordpress-plugin#82) to be installed on the connected site for most in-admin content tools — an older plugin without them makes those tools fail closed rather than silently falling back to direct REST. The exception is `wordpress_post_get` / `wordpress_post_update`, which instead require the community "Enable Abilities for MCP" plugin's abilities on the connected site (see the entry below) — an older/missing community plugin makes those two tools fail closed the same way.
